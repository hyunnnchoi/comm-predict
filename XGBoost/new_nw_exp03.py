...
model_avg = results_df.groupby('model')[['test_mae', 'test_mape']].mean().reset_index()
print("\nAverage performance by model across all configurations:")
print(model_avg.sort_values('test_mae'))

# Save model average metrics to CSV
model_avg.to_csv('results_by_model.csv', index=False)
print("Average performance by model saved to 'results_by_model.csv'")

# Calculate average metrics by worker count
worker_avg = results_df.groupby('left_out_worker')[['test_mae', 'test_mape']].mean().reset_index()
print("\nAverage performance by worker count:")
print(worker_avg)

# Save worker average metrics to CSV
worker_avg.to_csv('results_by_worker.csv', index=False)
print("Average performance by worker count saved to 'results_by_worker.csv'")

# Calculate average metrics by communication type
comm_avg = results_df.groupby('left_out_comm_type')[['test_mae', 'test_mape']].mean().reset_index()
print("\nAverage performance by communication type:")
print(comm_avg)

# Save communication type average metrics to CSV
comm_avg.to_csv('results_by_comm_type.csv', index=False)
print("Average performance by communication type saved to 'results_by_comm_type.csv'")
...
config_avg = results_df.groupby(['left_out_worker', 'left_out_comm_type'])[['test_mae', 'test_mape']].mean().reset_index()
print("\nAverage performance by configuration (worker + comm_type):")
print(config_avg.sort_values('test_mae'))

# Save configuration average metrics to CSV
config_avg.to_csv('results_by_worker_comm_config.csv', index=False)
print("Average performance by worker and communication type configuration saved to 'results_by_worker_comm_config.csv'")
...
model_pivot.to_csv('model_vs_config_pivot.csv')
print("Model vs Configuration pivot table saved to 'model_vs_config_pivot.csv'")
...
# Save the worker-comm pivot table to CSV
worker_comm_pivot.to_csv('config_vs_model_pivot.csv')
print("Configuration vs Model pivot table saved to 'config_vs_model_pivot.csv'")
...
importance_df.to_csv('feature_importance_overall.csv', index=False)
print("Overall feature importance saved to 'feature_importance_overall.csv'")

# Feature importance for each model type (averaged across worker counts and comm types)
print("\n===== Feature Importance by Model Type =====")
# Create a DataFrame to store all model-specific feature importances
...
for model_name in unique_models:
    ...
    # Add to the collection of all model importances
    all_model_importance.append(model_importance_df)

    for i, row in model_importance_df.iterrows():
        if row['Importance'] > 0.01:  # Only show features with non-trivial importance
            print(f"{row['Feature']}: {row['Importance']:.4f}")

# Combine all model importance DataFrames and save to CSV
if all_model_importance:
    combined_model_importance = pd.concat(all_model_importance)
    # Create a pivot table for easier analysis
    pivot_model_importance = combined_model_importance.pivot(index='Feature', columns='Model', values='Importance')

    # Save both formats
    combined_model_importance.to_csv('feature_importance_by_model_long.csv', index=False)
    pivot_model_importance.to_csv('feature_importance_by_model_wide.csv')
    print("Feature importance by model saved to 'feature_importance_by_model_long.csv' and 'feature_importance_by_model_wide.csv'")

# Feature importance by communication type
print("\n===== Feature Importance by Communication Type =====")
# Create a DataFrame to store all comm-type-specific feature importances
all_comm_importance = []
...
for comm_type in unique_comm_types:
    ...
    # Add comm type to DataFrame
    comm_importance_df['CommType'] = comm_type

    # Add to the collection of all comm type importances
    all_comm_importance.append(comm_importance_df)

    for i, row in comm_importance_df.iterrows():
        if row['Importance'] > 0.01:  # Only show features with non-trivial importance
            print(f"{row['Feature']}: {row['Importance']:.4f}")

# Combine all comm type importance DataFrames and save to CSV
if all_comm_importance:
    combined_comm_importance = pd.concat(all_comm_importance)
    # Create a pivot table for easier analysis
    pivot_comm_importance = combined_comm_importance.pivot(index='Feature', columns='CommType', values='Importance')

    # Save both formats
    combined_comm_importance.to_csv('feature_importance_by_comm_type_long.csv', index=False)
    pivot_comm_importance.to_csv('feature_importance_by_comm_type_wide.csv')
    print("Feature importance by communication type saved to 'feature_importance_by_comm_type_long.csv' and 'feature_importance_by_comm_type_wide.csv'")
