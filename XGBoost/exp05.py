...
def prepare_data(df, model_columns=None, dataset_columns=None):
    ...
    # Add pattern one-hot encoding if available
    pattern_dummies = None
    if 'Pattern' in df.columns:
        pattern_dummies = pd.get_dummies(df['Pattern'], prefix='pattern')

    # Add model architecture features if available
    model_arch_features = {}
    if 'Number of Parameters' in df.columns:
        model_arch_features['num_parameters'] = df['Number of Parameters'].astype(float)
    if 'Number of Layers' in df.columns:
        model_arch_features['num_layers'] = df['Number of Layers'].astype(float)
    ...
    # Calculate additional communication features
    N = df['Number of Workers']
    is_ring_allreduce = (df['Number of PSs'] == 0).astype(int)

    # Ring AllReduce features
    ring_volume = df['tensorsize'] * 2 * (N-1) / N * is_ring_allreduce
    ring_steps = 2 * (N-1) * is_ring_allreduce

    # PS features
    ps_mask = 1 - is_ring_allreduce
    ps_volume = df['tensorsize'] * N * 2 * ps_mask
    ps_worker_ratio = (N / df['Number of PSs'].clip(lower=1)) * ps_mask

    # Create list to collect all feature arrays
    feature_arrays = []

    # Add model one-hot encodings
    feature_arrays.append(model_dummies.values.astype(np.float32))

    # Add dataset one-hot encodings
    feature_arrays.append(dataset_dummies.values.astype(np.float32))

    # Add pattern one-hot encodings if available
    if pattern_dummies is not None:
        feature_arrays.append(pattern_dummies.values.astype(np.float32))

    # Add basic features
    feature_arrays.append(log_tensorsize.values.reshape(-1, 1).astype(np.float32))
    feature_arrays.append(N.values.reshape(-1, 1).astype(np.float32))
    feature_arrays.append(df['log_batch_size'].values.reshape(-1, 1).astype(np.float32))
    feature_arrays.append(df['Number of PSs'].values.reshape(-1, 1).astype(np.float32))

    # Add communication pattern features
    feature_arrays.append(is_ring_allreduce.values.reshape(-1, 1).astype(np.float32))
    feature_arrays.append(ring_volume.values.reshape(-1, 1).astype(np.float32))
    feature_arrays.append(ring_steps.values.reshape(-1, 1).astype(np.float32))
    feature_arrays.append(ps_volume.values.reshape(-1, 1).astype(np.float32))
    feature_arrays.append(ps_worker_ratio.values.reshape(-1, 1).astype(np.float32))

    # Add model architecture features if available
    for key, value in model_arch_features.items():
        feature_arrays.append(value.values.reshape(-1, 1).astype(np.float32))
